...
using LightningAgentMarketPlace.Lightning;
using LightningAgentMarketPlace.Chainlink;
using LightningAgentMarketPlace.AI;
using LightningAgentMarketPlace.Verification;
using LightningAgentMarketPlace.Acp;
using LightningAgentMarketPlace.Engine;
using LightningAgentMarketPlace.Engine.BackgroundJobs;
using LightningAgentMarketPlace.Engine.Queue;
...
using LightningAgentMarketPlace.Api.Authentication;
using LightningAgentMarketPlace.Api.Hubs;
using LightningAgentMarketPlace.Api.Middleware;
using LightningAgentMarketPlace.Api.HealthChecks;
using LightningAgentMarketPlace.Api.Lifetime;
using LightningAgentMarketPlace.Api.Services;
using LightningAgentMarketPlace.Data.Migrations;
...
{
    LightningAgentMarketPlace.Engine.Security.PreimageProtector.Initialize(escrowSettings.EncryptionKey);
}
...
builder.Services.AddScoped<LightningAgentMarketPlace.Engine.Services.MultiChainPriceService>();

// ── Payment Providers ────────────────────────────────────────────────
builder.Services.AddScoped<IPaymentProvider, LightningAgentMarketPlace.Engine.PaymentProviders.LightningPaymentProvider>();
builder.Services.AddScoped<IPaymentProvider, LightningAgentMarketPlace.Engine.PaymentProviders.Erc20PaymentProvider>();
builder.Services.AddScoped<IPaymentProvider, LightningAgentMarketPlace.Engine.PaymentProviders.NativeTokenPaymentProvider>();
builder.Services.AddScoped<IPaymentProvider, LightningAgentMarketPlace.Engine.PaymentProviders.CcipPaymentProvider>();
builder.Services.AddScoped<LightningAgentMarketPlace.Engine.PaymentProviders.PaymentRouter>();
...
LightningAgentMarketPlace.Lightning.LndTlsCertHandler.AllowInsecureDevelopmentMode = builder.Environment.IsDevelopment();
...
builder.Services.AddHttpClient<ICoinGeckoClient, LightningAgentMarketPlace.Engine.Services.CoinGeckoClient>();
...
builder.Services.AddHostedService<LightningAgentMarketPlace.Engine.BackgroundJobs.RecurringTaskService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (existingAgent is null)
    {
        ...
        var systemAgent = new LightningAgentMarketPlace.Core.Models.Agent
        {
            ExternalId = "system-builtin",
            Name = "System Worker Agent",
            Status = LightningAgentMarketPlace.Core.Enums.AgentStatus.Active,
        ...
        var skillTypes = new[]
        {
            LightningAgentMarketPlace.Core.Enums.SkillType.CodeGeneration,
            LightningAgentMarketPlace.Core.Enums.SkillType.DataAnalysis,
            LightningAgentMarketPlace.Core.Enums.SkillType.TextWriting,
            LightningAgentMarketPlace.Core.Enums.SkillType.ImageGeneration
        };

        foreach (var skill in skillTypes)
        {
            await capRepo.CreateAsync(new LightningAgentMarketPlace.Core.Models.AgentCapability
            {
                AgentId = agentId,
                SkillType = skill,
