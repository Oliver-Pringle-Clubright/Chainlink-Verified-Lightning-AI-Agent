...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> GetToken([FromBody] TokenRequest request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(configuredKey))
        {
            ...
            if (isAdminKey)
            {
                ...
                var adminAgent = new LightningAgentMarketPlace.Core.Models.Agent
                {
                    Id = 0,
                    ExternalId = "admin",
                    Name = "Admin"
    ...
    public async Task<IActionResult> RefreshToken([FromBody] RefreshRequest request)
    {
        ...
        if (int.TryParse(agentIdClaim, out var parsedId) && parsedId > 0)
        {
            ...
            if (dbAgent.Status != LightningAgentMarketPlace.Core.Enums.AgentStatus.Active)
        ...
        var agent = new LightningAgentMarketPlace.Core.Models.Agent
        {
