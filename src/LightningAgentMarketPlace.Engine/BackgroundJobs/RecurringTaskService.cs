...
public class RecurringTaskService : BackgroundService
{
    ...
    private async Task ProcessDueRecurringTasksAsync(CancellationToken ct)
    {
        ...
        var connectionFactory = scope.ServiceProvider.GetRequiredService<LightningAgentMarketPlace.Data.SqliteConnectionFactory>();
    ...
    private static async Task<List<RecurringTask>> GetDueRecurringTasksAsync(
        LightningAgentMarketPlace.Data.SqliteConnectionFactory factory, CancellationToken ct)
    {
    ...
    private static async Task UpdateRecurringTaskRunAsync(
        LightningAgentMarketPlace.Data.SqliteConnectionFactory factory, int id, DateTime lastRunAt, DateTime? nextRunAt, CancellationToken ct)
    {
        using var connection = factory.CreateConnection();
        using var cmd = connection.CreateCommand();
